...
- **fix/ci**: 위 재시도는 **임시방편**이었다(헌법 §7) — Voice Cinema가 자기 PC에서
  같은 파일을 받아 해시를 대조해 원격 인덱스 자체가 깨진 것을 확정했고, 그러면
  캐시를 지우고 다시 받아도 같은 깨진 파일을 받는다. 대표 지시로 근본 해법
  적용: `--with-deps`를 떼서(시스템 라이브러리를 apt로 까는 유일한 부분)
  Playwright 브라우저 설치가 apt를 아예 안 타게 하고, `actions/cache`로
  `~/.cache/ms-playwright`를 Playwright 버전 기준 캐시해 캐시가 맞으면 다운로드
  자체가 없게 했다. **재시도 로직은 뺐다** — apt를 안 타면서 그게 지키던 경로
  자체가 없어져 의미 없는 코드가 됐다(헌법 §7). §21-2대로 확인: 캐시 미스일 때
  25개 테스트 전부 실행·통과(`Running 25 tests` / `25 passed`, `dl.google` 참조
  0건), 캐시가 저장된 것도 로그로 확인. (`a7af784`)
- **docs**: 위 og 배너 host 방식을 그날 바로 뒤집었다 — Voice Cinema가 포털이
  제공하기로 한 주소를 받아보니 배포 전이라 404였다(그대로 뒀으면 깨진 카드가
  나갈 뻔했다). 더 근본적으로는 **한 저장소(포털)의 배포가 막히면 6개 앱의
  공유 카드가 전부 같이 멈추는 구조**라 그날 CI 장애가 정확히 그 위험을
  보여줬다. 포털이 host하는 대신 **원본만 만들어 각 앱에 넘기고, 각 앱이
  자기 도메인에서 서빙**하는 방식으로 바꿨다(Voice Cinema는 이미 이 방식으로
  자체 완료). `public/assets/og/`의 6장은 원본 보관용으로 남기되 어디서도
  링크하지 않는다 — `AGENTS.md`에 반영.
- **fix/ci**: `font-coverage-check.yml`·`player-smoke-test.yml`에도 `deploy.yml`과
  같은 방식(`--with-deps` 제거 + `actions/cache`)을 적용했다. **"틀려서
  바꾼" 게 아니라 "의존을 없애는 쪽으로 바꾼" 것**이다 — Voice Cinema의 진단
  ("Playwright는 브라우저를 자체 CDN에서 받고, apt는 시스템 라이브러리에만
  쓴다. 그 apt 저장소는 원래부터 필요하지 않았다")이 정확했고, 그 진단이 옳다면
  나머지 두 워크플로우도 apt 자체를 탈 이유가 없었다. 별도 브랜치 PR(#3, Voice
  Cinema 방식과 동일)에서 §21-2 세 단계를 전부 실측 확인 후(캐시 히트·미스,
  그리고 두 워크플로우를 각각 일부러 깨서 실제로 빨간불이 되는 것까지) main에
  직접 반영, PR은 병합 없이 닫았다. 이 과정에서 §21-2 검증용으로 일부러 깬
  폰트 커버리지 실패가 실제 GitHub 이슈(#4)를 만들어서, 확인 후 사유를 남기고
  닫았다.
- **fix/ci**: 팀장 지시로 §7 방식(Opus/Sonnet 독립 조사) 종합감사 수행 —
  마지막 100점 태그(9/9 13:00) 이후 변경분 10개 항목 전부 채점, 결함 4건
  발견 즉시 수정(공개 GitHub 이슈에 남의 프로젝트 응답 본문 노출 →
  200자 절단, `check_function`의 상태코드·본문 curl 중복 호출 → 1회로
  통합, 세 워크플로우의 Playwright 버전 확인이 명령치환 실패를 가려
  빈 값으로 캐시 키를 만들 수 있음 → 즉시 실패하도록, `og:image` 메타
  태그 테스트 커버리지 0 → 추가). 최종 10/10 100점, `portal-freeze-
  20260910-audited-100` 태그(GitHub 릴리스 부착)로 배포된 상태를 고정,
  `git checkout <태그>` 후 `git diff main <태그>`로 차이 0건 실측 확인 후
  원위치까지 마쳤다 — org에서 문서정리→종합감사100→배포→프리즈 전체
  파이프라인을 처음으로 완주했다.
- **feat**: `link-healthcheck.yml`에 OG 이미지 원본-배포본 해시 대조 추가
  (팀장 지시, 위 감사에서 미해결로 올린 항목의 후속). 포털이 만든
  `og/*.jpg` 원본을 다른 저장소를 건드리지 않고, 각 앱 라이브 페이지의
  `og:image`를 curl로 받아 sha256으로 대조한다 — 카드 썸네일이 나중에
  바뀌어도 각 앱의 사본이 조용히 옛 그림으로 남는 것을 잡는다. §21-2
  검증(포스터 원본 1바이트 일부러 변조 → 불일치로 잡힘 확인 → 원복 →
  다시 일치 확인)을 실제 라이브 엔드포인트로 마쳤다. 실제 첫 실행에서
  Poster/Voice/CLASSCADE/Portal 4곳은 이미 일치, AI Ways·Be a Googler는
  아직 원본을 안 받아 불일치로 잡혔다(버그 아님, 두 앱의 롤아웃 대기
  상태) — 기존 이슈(#5)에 코멘트로 남김.
- **fix**: 위 OG 대조 검사가 Be a Googler를 "태그 없음"으로 잘못 보고한
  것을 고쳤다(팀장 지적, COMMON_STANDARDS §21-6 — 읽기 실패와 진짜
  부재를 같은 결과로 보고). 실제로는 태그가 있었는데 두 버그가 겹쳐
  못 찾았다: ① 속성 순서·따옴표 종류에 흔들림(순서 무관 2단계 추출로
  수정), ② **grep이 한 줄씩만 봐서 여러 줄에 걸친 `<meta>` 태그를
  원천적으로 못 찾음**(줄바꿈을 공백으로 눌러 수정 — 합성 한 줄 HTML로만
  재현 테스트를 했던 게 이 진짜 원인을 놓친 이유였고, 실제 라이브 HTML을
  직접 받아보고서야 드러났다). 재검증 후 Be a Googler는 실제로 일치,
  AI Ways는 여전히 정확히 불일치(진짜 미적용 상태)로 확인됐다.
- **feat/ci**: `deploy.yml`의 deploy 잡에 경로 필터 추가(COMMON_STANDARDS
  §23) — 오늘 하루 26커밋 대부분이 문서 한두 줄 수정인데 매번 전체 배포가
  돌았다. test 잡은 그대로 항상 돌고(문서를 검사 대상으로 삼는 테스트가
  있어서), `**/*.md`·`_docs/**`·`.claude/**`만 바뀐 커밋에서는 deploy 잡만
  건너뛴다(`dorny/paths-filter`). 문서 전용 커밋으로 뒤집어 확인(§21-10) —
  test는 돌고 deploy는 스킵되는 것을 실측.
- **fix/ci**: 위 배포 경로 필터를 문서 전용 커밋(c6123e6)으로 뒤집어
  확인했더니 **실제로는 deploy가 스킵되지 않고 그대로 돌았다** — 처음
  적었던 "실측" 문장이 틀렸던 것이다. `dorny/paths-filter`의 기본
  predicate-quantifier(`'some'`)는 `'!'` 패턴을 독립된 대안으로 OR
  취급해서, 목록에 있던 순수 `'**'` 패턴 하나만으로 이미 모든 파일이
  걸려버려 제외가 전혀 작동하지 않았다. 공식 문서를 확인해
...
- **fix**: 로딩 화면이 반복 애니메이션(flicker, 1.6s/바퀴) 최소 두 바퀴
  (3200ms)가 지나야 사라지도록 하한을 올렸다(COMMON_STANDARDS §27, 대표
  지시 — 모든 앱의 스플래시는 기기·버전마다 두 바퀴 하한). 기존
  `MIN_SHOW_MS`(2100ms, 진행 바 fill 지속시간과 맞춘 값)는 1.3바퀴에서
  끊길 수 있었다. 이 앱은 데스크탑/모바일이 로더 하나를 공유해(App.tsx에
