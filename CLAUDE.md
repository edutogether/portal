...
- **상태**: **프리즈됨** — 최신 태그 `portal-freeze-20260826c`(그 이전 태그들은 옮기지 말고 보존: `20260813/14/14b/17/20/23/23b/24/24b/25/25b/824c/25c/25d/25e/25f/25g/25h/26/26b`). `edutogether.kr`는 2026-08-25에 GitHub 조직 도메인 인증(Verified) 완료됨.
...
- 카드 그리드 열 수(2026-08-26 최종 확정 — 같은 날 두 번 바뀜, 아래 이력 참고):
  **>1180px(데스크탑, 왼쪽 플레이어+오른쪽 카드 그리드) 2열**, ≤1180px(모바일 하단
  고정 플레이어로 전환) 2열, ≤560px 1열. 5·6번(아직 미배포 InKY 카드)이 화면
  아래쪽으로 밀려 덜 띄게 하고, 시각적 중심을 위쪽(플레이어+1·2번 카드)에 두려는
  대표의 명시적 결정 — 임의로 3열/4열 등으로 되돌리지 말 것.
  - **이력**: 카드 4개→6개가 되며 2열 그대로 뒀다가(2열/3행) 플레이어가 그 3행
    높이를 따라가 불필요하게 길어지는 사고가 나서 3열/2행으로 한 번 바꿨다가,
    대표가 "5·6번은 눈에 덜 띄게 아래로, 2열로 되돌려달라"고 다시 지정해 2열로
    재복귀. 이번엔 플레이어 높이 동기화 로직 자체를(아래 참고) "카드 1~4번(위쪽
    2행) 높이"로 바꿔서 2열이어도 플레이어가 안 길어지게 함.
  - **플레이어 높이 동기화**: `syncPlayerHeight`(JS)가 `.player` 높이를 카드
    그리드 전체가 아니라 **위쪽 2행(카드 1~4번) 높이**에만 맞춘다(`SYNC_ROWS = 2`
    상수). 그리드 전체 높이에 맞추면 카드 행이 늘 때마다 플레이어도 같이
    길어짐 — 처음엔 "1행만"으로 시도했다가 플레이어 내부(가사·컨트롤)가
    `overflow:hidden`에 잘려서 2행으로 조정했다(스크린샷으로 직접 확인한 실패
    사례이니 1행으로 되돌리지 말 것). 카드 5·6번(3번째 행)은 플레이어 하단보다
    더 아래로 자연스럽게 이어진다.
