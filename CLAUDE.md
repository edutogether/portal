...
— 2026-09-09에 AI Ways 주소를 바꾸며 정확히 그렇게 놓쳐서 한 번 되돌렸던 적이 있다(경위는
archive). 포털이 다른 앱의 내부 동작을 직접 검사하는 것은 범위를 넘으므로, 검사 대신
**그 앱 세션(또는 사람)의 확인**을 절차로 둔다. **다만 확인이 왔다고 끝이 아니다** — 자동
검사(헬스체크의 기능 검사 포함)가 초록불인 것은 "그 함수는 살아있다"는 뜻이지 "실제
사용자의 브라우저가 그 함수까지 도달할 수 있다"는 뜻은 아니다. 2026-09-09~10 AI Ways
건이 정확히 이랬다 — 서버 함수는 내내 정상이었는데, 한 번 거부당한 브라우저가 24시간
재시도를 안 하는 클라이언트 쪽 캐싱 때문에 특정 기기에서만 안 되는 것처럼 보였다.
자세한 경위는 `.claude/rules/app.md`의 "자주 틀리는 것" 참고.
