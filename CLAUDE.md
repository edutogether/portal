...
- **소리(2026-09-09 대표 확정, 8/31 결정을 뒤집음)**: 방문하면 **소리가 난다** — 볼륨 0에서 50%까지 부드럽게 올라오고, 음소거를 껐다 켜면 직전 값으로 부드럽게 돌아온다. 8/31에는 정반대로 "항상 음소거 시작"이 사양이었다(커밋 `3c8a48e`, 대표가 "소리가 저절로 나온다"고 지적). **음소거로 되돌리지 말 것** — 배경과 구현 제약(muted를 안 쓰는 이유, 페이드 목표값으로 토글 판정, 카드 클릭 예외)은 `.claude/rules/app.md`의 "소리" 절에 있다.
...
- **테스트**: Playwright 25개가 push/PR마다 CI에서 돈다 — 재생 스모크(`player.spec.js`),
