...
- **구성**: React + TypeScript + Vite. 소스 `src/`, 자산 `public/assets/`, **배포 대상은
  ...
  AGENTS.md 참고.
- **상태**: **프리즈됨, 실운영 모드.** 최신 태그는 아래 "프리즈 / 태그" 참고.
- **감사 주기(COMMON_STANDARDS §25, 2026-09-11 확정)**: 정기 종합감사는 **분기 말(3·6·9·12월
  말)**에 돈다 — 다음은 **2026-09-30**. 그 사이엔 **대표 지시가 있을 때만** 돌리고, 이 세션이
  먼저 재감사·재작업을 제안하지 않는다.
...
## 현재 링크 (2026-09-10 기준, 카드 DOM 순서와 동일)
...
**주소를 바꾸기 전에 해당 앱 세션(또는 사람)에게 새 오리진에서 기능이 실제로 되는지
확인받는다** — 상태코드 200과 본문 문자열 일치는 **정적 계층만** 보는 것이라 백엔드가
끊긴 것을 못 잡는다. 포털이 다른 앱의 내부 동작을 직접 검사하는 것은 범위를 넘으므로,
검사 대신 그 확인을 절차로 둔다. **다만 확인이 왔다고 끝이 아니다** — 자동 검사(헬스체크의
기능 검사 포함)가 초록불인 것은 "그 함수는 살아있다"는 뜻이지 "실제 사용자의 브라우저가
그 함수까지 도달할 수 있다"는 뜻은 아니다. 이 함정에 실제로 걸렸던 AI Ways 경위와 근거는
`.claude/rules/app.md`의 "자주 틀리는 것"과 `_docs/archive/history.md`에 있다.
