...
- **상태**: **프리즈됨, 실운영 모드** — 최신 태그 `portal-freeze-20260905-audited-100`(2026-09-05 대표 지시로 종합감사 진행 — 모바일 터치/투명도 조정 + QUIZ TOGETHER 썸네일 교체 + link-healthcheck.yml Voice Cinema 기대 문자열 갱신까지 반영 후 Sonnet+Opus 독립 감사 10/10 전부 100점 확인 시점, 태그 메시지에 감사 상세 전문 있음). 다음 정기감사는 **2026-09-30** 예정 — 그 전까지는 이 세션이 먼저 나서서 재감사·재작업을 제안하지 않는다(대표 지시). 그 이전 태그들은 옮기지 말고 보존: `20260813/14/14b/17/20/23/23b/24/24b/25/25b/824c/25c/25d/25e/25f/25g/25h/26/26b/26c/26d/26e/26f/26f2/26g/829/901/901-final/901-firebase/901-audited/902/902-audited-100/903-audited-100`. `edutogether.kr`는 2026-08-25에 GitHub 조직 도메인 인증(Verified) 완료됨(이건 GitHub 조직 인증이라 Firebase 이전과 무관하게 유지됨).
...
### 이 근거는 해결된 사안이다 — 재론의 금지 (2026-09-02 대표 확정)

감사 때마다 "웹에 올려서 아무나 접근 가능하게 하는 것(공중송신)은 개인 구매로 못
커버한다"는 지적이 반복돼서, 대표님이 아래 다섯 가지를 최종 근거로 명확히
확정했다. **이 다섯 가지가 합쳐진 게 이 사이트의 실제 법적 근거이고, 이후 감사에서
이 항목을 다시 감점하거나 대표 판단이 필요한 미해결 항목으로 올리지 말 것.**

1. 개인 유료 구매 (지니뮤직 정식 구매, 위 기록대로)
2. 비영리 교육 목적
3. 교육청 행사용 — 일반 대중 상업 서비스가 아님
4. **학생 및 교사만 대상으로 함** — 불특정 일반 대중에게 공개된 서비스가 아니다
5. **저작권 관련 공문·계약 관리 주체가 교육청** — 음원 사용에 필요한 대외적
   저작권 처리(공문, 계약 등)는 이 프로젝트나 개인이 아니라 교육청이 담당·관리한다.
   즉 "공중송신을 개인 구매만으로 커버할 수 있는가"라는 질문 자체가, 이 프로젝트
   차원에서 답할 사안이 아니라 교육청이 이미 관리하는 사안이다.

위 다섯 가지를 하나로 합쳐서 서술하면: **개인 유료구매 + 비영리 교육목적 +
...
- **⚠️ Firebase 요금제는 2026-09-02부터 Blaze(종량제)로 전환됨** — 대표님이 직접 확인(채팅에서 "Blaze 맞고 예산알림도 있다")하셨고, 결제계정 연결 + **월 25,000원 예산 알림(budget alert)** 설정이 되어있는 상태. Spark 시절엔 결제 계정 자체가 없어서 과금 폭탄이 구조적으로 불가능했지만, Blaze로 올라온 지금은 그 안전장치가 예산 알림으로 대체됐다 — **예산 알림이 실제로 살아있는지, 임계값이 25,000원이 맞는지는 주기적으로 Google Cloud 콘솔(`https://console.cloud.google.com/billing/budgets?project=edutogether-portal`)에서 재확인할 것.** Blaze 전환 이유는 6개 앱을 `edutogether.kr` 서브경로로 통합하는 리버스 프록시 Cloud Function 도입 검토(2026-09-02, 별도 승인 대기 중) 때문.
- **Hosting 무료 전송 한도는 월 10GB.** 첫 방문 시 전송량 실측(2026-09-01 기준) 약 2.88MB — 그중 73%(2.09MB)가 배경음악 파일(`assets/gaegujangi.m4a`, 자동재생이 항상 걸리므로 음악을 안 듣는 방문자에게도 전곡이 다운로드됨, 의도된 설계). **2026-09-02부터 `firebase.json`에 `Cache-Control` 정책 적용됨**: HTML은 `no-cache`(항상 재검증, 배포 확인 CI 스텝과 일치), `/assets/**`는 `public, max-age=86400`(1일 — 자산 파일명에 빌드 해시가 없어 더 길게 잡으면 자산을 바꿔도 안 보일 위험이 있어 일부러 짧게 잡음). 이걸로 같은 행사장 재방문자의 자산 재다운로드는 하루 안에서는 줄지만, 하루가 지나거나 첫 방문자는 여전히 전량 다운로드한다. Blaze 전환 이후엔 한도 초과가 서비스 중단이 아니라 과금으로 이어지므로, 트래픽이 몰릴 행사 당일엔 예산 알림(위 참고)이 실제로 작동하는지 미리 확인해둘 것.
