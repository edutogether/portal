...
export default defineConfig({
  testDir: './tests',
  timeout: 15000,
  // CI에서만 워커 1로 — 2026-09-09에 2워커에서 서로 무관한 테스트 4개가 한꺼번에
  // 흔들린 것을 봐서 내렸다. 원인 제거가 아니라 비결정성 제거다: 정확한 방아쇠는
  // 못 찾았다(.claude/rules/app.md에 조사 경위 있음). 이 저장소는 코드 3,286줄에
  // 테스트 파일 7개라 워커 1로 잃는 CI 시간은 1분 남짓이고, 무작위로 빨간불이
  // 뜨는 게이트는 진짜 결함이 났을 때도 "또 그거네"로 재실행당해 게이트 구실을
  // 못 한다는 쪽이 더 큰 손해라고 판단했다. 로컬은 지금대로 병렬 유지(빠른
  // 반복이 우선). 워커 1로 내린 뒤에도 같은 흔들림이 또 나오면 경합이 원인이
  // 아니라는 뜻이니 그때 다시 판다.
